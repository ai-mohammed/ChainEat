...
function Restaurants() {
  ...
  function StarRating({ restaurant }: { restaurant: Restaurant }) {
    ...
    if (localRating > 0) {
      ...
      ratingText = `Avg: ${restaurant.rating.toFixed(1)}/5 (${restaurant.ratingCount})`;
    }
  ...
  const sortRestaurants = (arr: Restaurant[]): Restaurant[] => {
    ...
    return [...arr].sort((a, b) => {
      let valA = sortKey === "name" ? a.name : a.rating || 0;
      let valB = sortKey === "name" ? b.name : b.rating || 0;
    ...
    <div style={{ width: "100%", padding: "80px 20px" }}>
      ...
      <div style={{ display: "flex", gap: "1rem", justifyContent: "center", marginBottom: "30px" }}>
        ...
        <select
          style={{ padding: "0.5rem", borderRadius: "6px", border: "1px solid #ccc" }}
          value={sortKey}
          onChange={(e) => setSortKey(e.target.value as "none" | "name" | "rating")}
        >
            ...
            >
              <div style={{ padding: "16px" }}>
                ...
                <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center" }}>
                  <h2 style={{ margin: "0 0 8px", color: "#ff6600" }}>{restaurant.name}</h2>
                  <button
                    style={{
                ...
                {isExpanded && (
                  <div style={{ marginTop: "10px", color: "#444" }}>
                    {restaurant.description ? (
                      <p style={{ whiteSpace: "pre-wrap" }}>{restaurant.description}</p>
                    ) : (
